#include <bits/stdc++.h>

using namespace std;

const int MAXN = 1e5;


vector<int> adj[MAXN];
vector<bool> processed(MAXN);
vector<int> fix(MAXN);
vector<int> low(MAXN);
vector<int> used(MAXN);
vector<int> excentials;
int counter;
int n;

void excential_node(int node){
	if(used[node] == 0){
		excentials.push_back(node);
		used[node] = 1;
	}
}

void dfs(int node, int father = -1){
	int children = 0;
	processed[node] = true;
	fix[node] = low[node] = counter ++;
	for(int element : adj[node]){
		if(element == father){
			continue;
		}
		if(processed[element]){
			low[node] = min(low[node],fix[element]);
		} else{
			dfs(element,node);
			low[node] = min(low[node], low[element]);
			if(low[element] >= fix[node] && father != -1){
				excential_node(node);
			}
			children++;
		}
	}
	if(children > 1 && father == -1 ){
		excential_node(node);
	}
}

int main(){
	int n,m;
	cin >> n >> m;
	for (int i = 0; i < m; i++){
		int a,b;
		cin >> a >> b;
		adj[a].push_back(b);
		adj[b].push_back(a);
	}
	dfs(1);
	cout << excentials.size() << endl;
	for(auto e : excentials){
		cout << e << endl;
	}
}