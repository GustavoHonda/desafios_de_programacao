...
vector<int> min_route(int n , int m, int init_vertex, vector<vector<pair<int,int>>> &adj){
	pair<int,int> edge;
	priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>>> fila;
	vector<int> cumulative_cost(n+1,INT_MAX);
	...
	while(fila.size() > 0){
		...
		for(pair<int,int> p : adj[destiny]){
			int cost_2 = p.second + cumulative_cost[destiny];
...
int main(){
	...
	vector<int> trip_cost = min_route(n,m,1,adj);
	vector<int> trip_inv_cost = min_route(n,m,n,adj_inv);
	...
	for(int i = 1; i<= n; i++){
		for(pair<int,int> p : adj[i]){
			...
			if(trip_cost[origin] == INT_MAX || trip_inv_cost[destiny] == INT_MAX){
				continue;
			}
